...
Discussion: [container-registry/harbor-next#493](https://github.com/container-registry/harbor-next/pull/493) (core dispatch refactor), [container-registry/harbor-next#480](https://github.com/container-registry/harbor-next/issues/480) / [#481](https://github.com/container-registry/harbor-next/pull/481) (locked-user fix), [container-registry/harbor-next#482](https://github.com/container-registry/harbor-next/issues/482) / [#483](https://github.com/container-registry/harbor-next/pull/483) (registry proxy credential passthrough), [container-registry/harbor-next#497](https://github.com/container-registry/harbor-next/pull/497) (v2 Basic/Bearer challenge fix)

Related upstream issues (not being addressed via goharbor/harbor directly — see Implementation): [goharbor/harbor#1572](https://github.com/goharbor/harbor/issues/1572), [#13372](https://github.com/goharbor/harbor/issues/13372), [#21300](https://github.com/goharbor/harbor/issues/21300), [#7965](https://github.com/goharbor/harbor/issues/7965), [#7964](https://github.com/goharbor/harbor/issues/7964), [#21853](https://github.com/goharbor/harbor/issues/21853)
...
- **Genuinely concurrent multi-backend authentication** (e.g. trying LDAP then OIDC for the same login attempt, or per-user backend assignment). Dispatch still resolves to exactly one backend per priority order. #13372/#21853 are only partially addressed — this proposal makes leftover multi-backend config *visible* to operators, it does not implement true concurrent backend support, which would need its own design (per-user backend resolution, group collision handling, security review).
...
This was originally proposed upstream to goharbor/harbor as a single 16-commit PR ([#23458](https://github.com/goharbor/harbor/pull/23458), recreated as [#23483](https://github.com/goharbor/harbor/pull/23483)) bundling security fixes, the registry proxy change, and the core dispatch refactor together across 60+ files. It was closed without merge — maintainer feedback was that it was "not testable; too big, too many things at the same time" with "no discussion or community consensus." Rather than re-propose the same bundle upstream, this work now targets `container-registry/harbor-next` directly, split into independent PRs so each stands on its own review:

- **[container-registry/harbor-next#481](https://github.com/container-registry/harbor-next/pull/481)** — Locked user returns `ErrAuth` instead of `(nil, nil)`. Self-contained, no dependency on the rest.
- **[container-registry/harbor-next#497](https://github.com/container-registry/harbor-next/pull/497)** — `/v2` Basic-vs-Bearer challenge dispatch fix. Self-contained.
- **[container-registry/harbor-next#483](https://github.com/container-registry/harbor-next/pull/483)** — Registry proxy per-user credential exchange, probe-based auth detection, per-scope token caching. Self-contained, no file overlap with the core refactor below.
- **[container-registry/harbor-next#493](https://github.com/container-registry/harbor-next/pull/493)** — The core `Match()`-based dispatch refactor this proposal is actually about: per-backend `Match()` implementations, `ConfiguredAuthBackends`/`DetectAuthMode`, and the settings-UI visibility work (surfacing when more than one backend has leftover config). Depends on nothing above merging first, but is cleaner to review once the unrelated fixes above are out of the way.

#481, #497, and #483 need no design consensus — they're pure bugfixes independent of this proposal's architecture question. #493 is the PR this proposal is actually seeking discussion on.
...
- **Implementation**: [container-registry/harbor-next#493](https://github.com/container-registry/harbor-next/pull/493) (core), [#481](https://github.com/container-registry/harbor-next/pull/481), [#483](https://github.com/container-registry/harbor-next/pull/483), [#497](https://github.com/container-registry/harbor-next/pull/497)
- **Original upstream attempt (closed, not merged)**: [goharbor/harbor#23458](https://github.com/goharbor/harbor/pull/23458) / [#23483](https://github.com/goharbor/harbor/pull/23483)
- **Related upstream issues**: goharbor/harbor#1572, #13372, #21300, #7965, #7964, #21853
